...
object Anagrams {
   ...
   */
  def wordOccurrences(w: Word): Occurrences = (w map (_.toLower) groupBy (c => c) mapValues { x => x.length } toList).sortBy(_._1)
  ...
  def combinations(occurrences: Occurrences): List[Occurrences] = occurrences match {
    ...
    case x :: xs =>
      (for {
        occ <- combinations(xs)
        count <- x._2 until 0 by -1
        if (count > 0)
      } yield (x._1, count) :: occ) ::: combinations(xs)
  }
  ...
  def subtract(x: Occurrences, y: Occurrences): Occurrences = {
    (y.foldLeft(x.toMap) { case (occurrences, (char, count)) => occurrences.updated(char, occurrences(char) - count) } filter (_._2 > 0) toList).sortBy(_._1) 
  }
    ...
    def occurrenceAnagrams(occurrences: Occurrences): List[Sentence] = {
      if (occurrences.isEmpty) List(List())
        ...
        for {
          ...
          sentence <- occurrenceAnagrams(subtract(occurrences, comb))
        } yield word :: sentence
    }
